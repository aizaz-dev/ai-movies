      ...
      {box.map((item)=>{
        return   <div key={item.id} 
             ...
             <div className=" w-[90%] border-b-[1px] ml-2 border-b-gray-300 mb-[30px]"></div>
                <div className="text-sm  text-primary font-bold font-[Outfit-Bold] max-md:text-left max-laptop:text-mf">{item.name}</div>
                <div className="text-sm max-md:text-mf leading-7 max-laptop:text-[15px] max-laptop:leading-4 max-md:leading-5 max-md:text-start">{item.des}</div>
                
                  <div className='flex gap-[20px] mt-[30px]'>
<img className='w-[80px] h-[80px]' src={item.userpic} alt="" />
<div className='flex flex-col items-start justify-center'>
  <h3 className='text-sm  text-primary font-[Outfit-Bold] max-md:text-left max-laptop:text-mf'>{item.username} </h3>
  <p className="text-sm max-md:text-mf font-[Outfit-Regular] leading-7 max-laptop:text-[15px] max-laptop:leading-4 max-md:leading-5 max-md:text-start">{item.userinfo}</p>
</div>
                  </div>
       </div>
       })}
